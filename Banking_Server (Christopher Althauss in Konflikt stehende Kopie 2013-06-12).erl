...
-export([start/0, konto_anlegen/1, lookup/1, return/1]).

% These are all wrappers for calls to the server
start() -> gen_server:start_link({bank, ?MODULE}, ?MODULE, [], []).
konto_anlegen(ClientPID) -> gen_server:call(ClientPID, {konto_anlegen}).	
lookup(Book) -> gen_server:call(?MODULE, {lookup, Book}).
return(Book) -> gen_server:call(?MODULE, {return, Book}).

% This is called when a connection is made to the server
init([]) ->
	Library = dets:open_file(food, [{type, bag}, {file, "/bank"}]).,
	{ok, Library}.

% handle_call is invoked in response to gen_server:call
handle_call({konto_anlegen}, ClientPID, Library) ->
