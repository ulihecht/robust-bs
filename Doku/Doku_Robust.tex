\documentclass[10pt,a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{color}
\usepackage[ngerman]{babel}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{makeidx}
\usepackage{booktabs}
\usepackage{here} 
\usepackage{graphicx}
\graphicspath{ {./images/} }
\usepackage{multirow}
\usepackage{multicol}
\usepackage[hang]{footmisc}
\usepackage[lmargin=3.0cm,rmargin=3.0cm,tmargin=3.0cm,bmargin=3.0cm,head=2cm,headsep=0.5cm]{geometry}
\linespread{1.5}
\usepackage{listings} \lstset{numbers=left, numberstyle=\tiny, numbersep=5pt} 
\definecolor{code}{rgb}{0.94, 0.97, 1.0}
\definecolor{red}{rgb}{1.0, 0.03, 0.0}
\definecolor{green}{rgb}{0.0, 0.65, 0.31}
\usepackage{hyperref}
\lstset{
	  literate={Ö}{{\"O}}1 {Ä}{{\"A}}1 {Ü}{{\"U}}1 {ß}{{\ss}}2 {ü}{{\"u}}1 {ä}{{\"a}}1 {ö}{{\"o}}1 {µ}{\textmu}1,
      columns=fullflexible,
      language=Erlang,
      showstringspaces=false,
      commentstyle={\color{green}},
      keywordstyle=\color{black},
      stringstyle=\color{black},
      numbers=none,
      extendedchars=\true,
      tabsize=4,
      breaklines=true,
      breakatwhitespace=true,
      frame=none,
      backgroundcolor=\color{code}} 

\title{Banking Server}

\begin{document}

\begin{titlepage}
\vspace*{1cm}
\begin{center}
\Huge
Technische Hochschule Nürnberg\\
\vspace*{2cm}
\large
Projektarbeit\\ Robuste Software für parallele Computerarchitekturen\\
Dozent: Prof. Dr. Axel Hein\\
\vspace*{2cm}
\Huge
Banking Server\\
\vspace{1cm}
\large
\vspace{2cm}

 \begin{tabular}{p{6 cm}p{6 cm}}
    	vorgelegt von & {Christopher Althaus} \\
		& {Markus Endres} \\
		& {Alexander Baumgärtner} \\
		& {Christoph Bohner} \\
		& {Ulrich Hecht} \\ & \\
    	Erstgutachter & {Prof. Dr. Axel Hein}\\ & \\
    	Abgabe:& 27. Mai 2013
 \end{tabular}\\
    


\end{center}
\end{titlepage}

\clearpage\thispagestyle{empty}
\begin{center}\textbf{\large Erklärung}\end{center}

\noindent
Hiermit versichern wir, dass wir die Arbeit selbständig verfasst, nicht anderweitig für Prüfungszwecke vorgelegt, alle benutzten Quellen und Hilfsmittel angegeben 
sowie wörtliche und sinngemäße Zitate als solche gekennzeichnet zu haben. \\
\\ \\ \\ \\
\begin{tabular}{l}
 \\ \\ \\
\line(1,0){165}\\
Markus Endres\\
 \\ \\ \\
\line(1,0){165}\\
Alexander Baumgärtner\\
 \\ \\ \\
\line(1,0){165}\\
Christoph Bohner\\
 \\ \\ \\
\line(1,0){165}\\
Ulrich Hecht\\
 \\ \\ \\
\line(1,0){165}\\
Christopher Althaus \qquad \qquad \qquad \qquad \qquad  \qquad  \qquad  \qquad \qquad  \qquad   Erlangen, den \today
\end{tabular}



%  Abbildungsverzeichnis 
%  Zuständigkeiten und Tätigkeiten der einzelnen Projektteilnehmer 
%  Einleitung und Motivation  
%  Design und Architektur 
% Implementierung 
% Fallstudie (exemplarische Anwendung der Software) 
% Zusammenfassung und Fazit 
% Glossar (Erklärungen der Abkürzungen und Fachbegriffe) 
% Literatur- und Quellenangaben 
% Index 
% Bedienungsanleitung (User Guide) 
% Anhang 

\newpage
\tableofcontents
\newpage
%\section{Vorwort/Motivation} % Christoph
\section{Teamorganisation}	% Christoph & Alex
Die Projektgruppe 3 bestand aus Markus Endres, Christoph Bohner, Ulrich Hecht, Christopher Althaus sowie Alexander Baumgärtner. Inerhalb der Gruppe wurde festgelegt dass eine flache, hierarchiefreie Organisation herschen soll
dies bedeutet auch, dass alle Teammitglieder gleichermaßen Aufgaben zu bearbeiten hatten. Für die
erste Präsentation wurden die Themen verteilt, sodass jeder einen Aufgabenteil zu bearbeiten hatte.
Da sich diese Art der Organisation bewährte hatten wir diesen Weg auch zur Erstellung dieser
Arbeit gewählt. Ulrich Hecht und Christopher Althaus wurde die Ausarbeitung des Clients sowie die Erstellung des Servers übertragen. Christoph Bohner bearbeitete das Profiling, als auch die Erstellung der Abschlusspräsentation. Markus Endres und Alexander Baumgärtner befassten sich mit der Programmierung des Workers.
%\newpage
%\input{Design_Architektur_Server}  	% Serverseite: Ulli 
%\input{Design_Architektur_Worker}  	% Worker: Markus
\section{Implementierung}
\input{Bedienungsanleitung}
\input{Client}						% Christopher
\input{Server}						% Christopher
\input{Worker_Teil_Alex}
%\input{Worker_Teil_Markus}
%\input{Profiling}					% Christoph
%\input{Bedienungsanleitung}			% Uli
%\input{LiteraturQuellenangaben}		% Alle
%\input{Fazit}
%\input{Anhang}
\end{document}